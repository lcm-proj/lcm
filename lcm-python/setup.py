...
pkgconfig_cflags = commands.getoutput ("pkg-config --cflags glib-2.0 lc")
pkgconfig_include_flags = commands.getoutput ("pkg-config --cflags-only-I lc")
pkgconfig_include_dirs = [ t[2:] for t in pkgconfig_include_flags.split() ]

pkgconfig_lflags = commands.getoutput ( \
        "pkg-config --libs-only-l glib-2.0 lc")
...
pylcm_extension = Extension("lcm._lcm",
                           sources=["pylc.c", "pyeventlog.c", "pylcm.c"],
